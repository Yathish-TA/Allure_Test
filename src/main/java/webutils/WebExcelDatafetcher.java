...
public class WebExcelDatafetcher extends CustomHtmlReport {
    private static final Logger log =
            LogManager.getLogger(helperfunctions.ExcelDataProvider.class);

    public WebExcelDatafetcher() {
        this.commonPojo = new CommonPojo();
    }

    public static int headerIteration(XSSFSheet sheet, int row,
                                      List<String> headers) {
        if (!headers.isEmpty()) {
            headers.clear();
        }
    ...
    public static List<Map<String, Object>> readExcel(String Workbook,String Worksheet) {
        String excelWorkBookPath = "src//test//java//Web//resources//testdata//" + Workbook;
    ...
    public static Map<String, String> getExcelData(String dataref,String Workbook,String Worksheet) {
        ...
        for (Map<String, Object> map : tdList) {
            ...
            (map.get("Execute").toString().equalsIgnoreCase("Yes") &&
                            map.get("TestCaseID").toString().equalsIgnoreCase(dataref)) {
    ...
    public static Map<String, String> convertIteratorToMap(Iterator<Object[]> iterator) {
